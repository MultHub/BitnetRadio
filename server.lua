--  oh lookie, it's the same code everyone uses
--  
...
--                       ^ bit                                         ^ bit
--  bitbucket exists just to host 1 file (bit.lua) CONFIRMED
--  
...
local locked = conf.password ~= nil
...
local cmds = {
  ...
  lock = {"", "Lock server console", fn=function()
    if conf.password then
      locked = true
    end
  end},
  setpass = {"[password]", "Set or clear lock password", "Set lock password\nNo arg: reset password", fn=function(password)
...
function cmds.help.fn(command)
  if command and not tonumber(command) then
    ...
    print("Escape strings with \\")
    for i = 1 + (page - 1) * (h - 3), page * (h - 3) do
      if cmdHelp[i] then
        print(cmdHelp[i])
      end
    end
...
parallel.waitForAny(function()
  ...
  while running do
    if locked and conf.password ~= nil then
      ...
      write("Password: ")
      local str = read("*")
      if sha256(str) == conf.password then
        locked = false
        term.clear()
        term.setCursorPos(1, 1)
        if term.isColor() then
